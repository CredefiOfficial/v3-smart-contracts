...
contract ProfitShare is Ownable
{
    ...
    address immutable public MODULEX;
    ...
    constructor(uint96 start_epoch, uint96 end_epoch, uint reward_amount, address _modulex_addr, address _rewards_token) Ownable(_msgSender())
    {
        ...
        pool = PoolInfo({
            staking_token: IERC20(IModuleX(MODULEX).getxCREDIAddress()),
    ...
    function _stake(address owner, uint96 lock_period, uint amount, uint stake_id) internal
    {
        ...
        emit StakeUpdated(stake_id, owner, user_stake.stake_amount, user_stake.modulex_stake_id, user_stake.maturity, lock_period);
    }
    ...
    function stakeCREDI(uint96 lock_period, uint CREDI_amount) external
    {
        ...
        stakes[_msgSender()][stake_id].modulex_stake_id = modulex_stake_id;
        _stake(_msgSender(), lock_period, amount, stake_id);  
    }
    ...
    function unstake(uint stake_id, bool claim_rewards) validate_stake(_msgSender(), stake_id) external
    {
        ...
        if(user_stake.modulex_stake_id > 0)
        {
            require(IModuleX(MODULEX).claim(user_stake.modulex_stake_id) == user_stake.stake_amount, "ProfitShare:Cannot claim xCREDI from ModuleX!");
        }    
